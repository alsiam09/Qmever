    ...
    <div className="BuyProDuct w-[100%] md:w-[40%]">
      {BuyItem.map((Buyitemuser) => (
        Buyitemuser.totalPrice
          ...
          <div className="">
            {(Buyitemuser.cartItem).map((item)=>(
             ...
             <h2 className=' pr-[10px] sm:border-r-[1px] flex items-center h-[70px] sm:border-x-[#fff] text-[#fff]'>{item.Prodectname}</h2>
             {
               item.ModelName 
               ?
               <h2 className=' pr-[10px] sm:border-r-[1px] flex items-center h-[70px] sm:border-x-[#fff] text-[#fff]'>{item.ModelName} : {item.size}</h2>
               :
               ""
             }
             <h2 className=' pr-[10px] sm:border-r-[1px] flex items-center h-[70px] sm:border-x-[#fff] text-[#fff]'>ProdectQun x {item.ProdectQun}</h2>
             <h2 className=' pr-[10px] flex items-center h-[70px] text-[#fff]'>Price : {item.Pricesub !== "" && item.Pricesub ? item.Pricesub : item.Price}৳</h2>
         </div>
       ...
       <div className='flex items-center gap-[10px]'><h3 className=' w-[20px] h-[20px] rounded-[50%] border-[2px] border-[red]' ></h3><h3 className='text-[18px] text-[#fff]'>Cash on delivery</h3></div>
         ...
         <h2 onClick={HandleSendOTP} className="pay w-[100%] my-[40px] md:mx-[10px] h-[40px] text-[#fff] font-[700] flex justify-center items-center text-[18px] bg-[red] rounded">Pay Now {Buyitemuser.Pricesub !== "" && Buyitemuser.Pricesub ? Buyitemuser.Pricesub : Buyitemuser.Price}৳</h2>
         </div>
